...
3. **PPTX is an exit door, never a format target.** Export is lossy by design and fidelity is not promised. The core will never contain an importer for arbitrary PPTX or let PowerPoint's model shape sldr formats. Importing arbitrary decks is agent work — a capable model translating them into canonical slides/layouts/flavors — producing plain files, not a core parser. The one deterministic exception is the round-trip of sldr-generated decks, which is the inverse of the export table and lives in the `sldr-pptx` satellite crate (ADR-0010).
