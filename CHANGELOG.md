...
- `skeleton` flavor — a neutral wireframe lens for showing what a layout *is*, independent of any brand: system fonts (no web-font fetch, renders instantly), grayscale, dashed outlines on the structural regions (content/columns/framed zones/image cells). Pair with the reference deck (`sldr build reference --flavor skeleton`) for the layout catalog, or use it to render fast thumbnails.
