...
install-system:
    ...
    for bin in sldr sldr-server; do
...
# Start the sldr server (development)
serve:
    cargo run -p sldr-server

# Start the sldr server (release)
serve-release:
    cargo run -p sldr-server --release
